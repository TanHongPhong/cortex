...
| ----- | ----- | ------------- | ---------- | --------------- |
| `/` | MVP | Giới thiệu CORTEX, dẫn vào khóa học/contact | `courses`, [[web/page/website/projects|`projects`]], `course_reviews`, `leads` nếu có form | `/courses`, `/contact` |
...
| Route | Phase | Vai trò chính | Data chính | CTA / Next step |
| ----- | ----- | ------------- | ---------- | --------------- |
| `/login` | MVP | Đăng nhập bằng email/password cho student/admin/instructor/course_editor | `users`/auth | student -> `/dashboard`, instructor -> `/instructor`, course_editor -> `/admin/courses`, [[web/page/admin/admin|admin]] -> `/admin` |
| `/register` | MVP | Tạo user role student | `users`, optional [[web/page/student/referral|referral]] param | `/dashboard` hoặc `/login` |
...
| `/learn/[course]/[lesson]` | MVP + P1 | Học video/resource, quiz, Q&A, nộp assignment/final_project | `lessons`, `video_assets`, `lesson_resources`, `files`, `lesson_progress`, [[web/page/instructor/submissions|`submissions`]], `quizzes`, `quiz_questions`, `quiz_attempts`, `lesson_questions` | Complete/Submit/Ask/Next |
...
| `/checkout/:courseSlug` | MVP | Tạo order mua khóa đơn | `users`, `courses`, `orders`, `coupons`, `referral_codes`, `payment_transactions` | `/checkout/success` hoặc `/checkout/failed` |
| `/checkout/success` | MVP | Kết quả thanh toán online thành công hoặc pending gateway confirmation | `orders`, `payment_transactions`, `enrollments`, [[web/page/student/notifications|`notifications`]] | `/learn/[course]` hoặc `/my-orders/:id` |
| `/checkout/failed` | MVP | Kết quả thanh toán lỗi/hủy | `orders`, `payment_transactions` | retry [[web/page/student/checkout|checkout]] hoặc `/my-orders/:id` |
...
| `/coupon` | Optional/P1 | Xem/copy [[web/page/student/coupon|coupon]] nếu cần | `coupons`, `coupon_redemptions`, `orders` | `/checkout/:courseSlug` |
| `/referral` | Future | P2, mã giới thiệu và reward | `referral_codes`, `referral_conversions`, `orders` | copy invite link |
...
| ----- | ----- | ------------- | ---------- | ------------ |
| `/instructor` | P1 | Tổng quan khóa được phân công | `users`, `course_instructors`, `courses`, [[web/page/instructor/submissions|`submissions`]], `lesson_questions`, [[web/page/student/notifications|`notifications`]] | vào queue cần xử lý |
| `/instructor/courses` | P1 | Xem khóa/module/lesson (read-only) | `course_instructors`, `courses`, `modules`, `lessons` | xem curriculum read-only; không sửa lesson/upload video/announcement |
...
| `/instructor/questions` | P1 | Trả lời Q&A khi `can_answer_questions = true` | `course_instructors`, `lesson_questions`, `users`, `courses`, `lessons`, [[web/page/student/notifications|`notifications`]] | answer/mark resolved/hide |
...
| Route | Phase | Vai trò chính | Data chính | Action chính |
| ----- | ----- | ------------- | ---------- | ------------ |
| `/admin` | MVP + P1 | Tổng quan vận hành + commerce/P1 alerts | `users`, `courses`, `resources`, `enrollments`, `orders`, [[web/page/instructor/submissions|`submissions`]], `certificates`, `leads`, `lesson_questions`, `course_reviews`, [[web/page/student/notifications|`notifications`]] | điều hướng xử lý |
| `/admin/courses` | MVP | Quản lý vỏ khóa học; `course_editor` được vào content-only | `courses`, `course_faqs`, `course_instructors`, count `lessons`, count `enrollments`, `course_reviews` | admin: create/edit/assign instructor/publish/archive; course_editor: create/edit content, manage lessons |
| `/admin/resources` | P1 | Quản lý public resources/blog hub | `resources`, `files`, `courses`, `leads` | create/edit/publish/archive |
| `/admin/lessons` | MVP + P1 | Quản lý module/lesson/video assets/resources/quiz; `course_editor` được vào content-only | `courses`, `modules`, `lessons`, `video_assets`, `lesson_resources`, `files`, `quizzes`, `quiz_questions` | admin/course_editor: create/edit/reorder content; admin giữ quyền publish/archive nhạy cảm |
...
| `/admin/orders` | MVP | Quản lý tiền/order tổng, gateway status và refund credit | `orders`, `users`, `courses`, `payment_transactions`, `invoices`, `coupons`, `account_balance_transactions`, `audit_logs` | view gateway transactions/refund to balance/export |
...
| Course editor role | `/admin/courses*`, `/admin/lessons*` content-only với sidebar lọc | kế thừa instructor hoặc vào full admin |
