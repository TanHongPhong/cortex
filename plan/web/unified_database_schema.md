...
| `created_by`        | UUID      | FK → `users.id`  | Người tạo ([[web/page/admin/admin|admin]]/system)           |
...
**Business Rule:** Refund chỉ cộng credit nội bộ vào `users.account_balance`; balance không dùng để [[web/page/student/checkout|checkout]]/mua khóa trong MVP/P1. User muốn rút tiền phải liên hệ [[web/page/admin/admin|admin]], [[web/page/admin/admin|admin]] xử lý offline rồi tạo transaction `admin_withdrawal_reset` hoặc `admin_adjustment_reset` để đưa balance về `0`.
...
`files` lưu metadata cho file nội bộ như avatar, thumbnail, tài liệu, attachment, [[web/page/website/certificate|certificate]] PDF và invoice PDF. File binary nằm ở object storage/CDN, không nằm trong database.

| Field             | Type      | Constraint                         | Description                                      |
| :---------------- | :-------- | :--------------------------------- | :----------------------------------------------- |
| `id`              | UUID      | PK                                 | Định danh duy nhất                               |
| `owner_type`      | String    | -                                  | Entity sở hữu: `user`, `course`, `lesson`, `submission`, [[web/page/website/certificate|`certificate`]], `invoice`, `order` |
...
| `course_id`         | UUID      | **Not Null**, FK → `courses.id` ON DELETE RESTRICT | Khóa học                                      |
...
| `note`              | Text      | -                                                 | Ghi chú của [[web/page/admin/admin|admin]]                             |
...
**Composite Index**: `(course_id, status)` - Optimize [[web/page/admin/admin|admin]] queries theo analysis/status.
...
| Nhóm dữ liệu | Bảng chính | FK delete policy | Lý do |
| :----------- | :--------- | :--------------- | :---- |
| Configuration / draft content | `modules`, `lessons`, `lesson_resources`, `course_faqs` | Có thể `CASCADE` cho child content khi parent chỉ hard-delete ở trạng thái draft/empty | Dữ liệu cấu trúc chưa phát sinh lịch sử có thể dọn cùng parent |
| Learning history | `enrollments`, `lesson_progress`, `submissions`, `quiz_attempts`, `certificates` | `RESTRICT` / `NO ACTION` | Giữ lịch sử học tập, bài nộp, chứng chỉ và khả năng verify |
| Commerce history | `orders`, `payment_transactions`, `invoices`, `coupon_redemptions`, `referral_conversions`, `account_balance_transactions` | `RESTRICT` / `NO ACTION`, nullable analytics có thể `SET NULL` | Không mất dữ liệu tài chính, snapshot, ledger và audit |
| Audit / operations history | `audit_logs`, `payment_webhook_logs`, `notifications` | `RESTRICT` / `NO ACTION`, archive bằng policy riêng nếu cần | Log là bằng chứng vận hành, không xóa qua cascade |

**Hard delete rule:** Hard delete chỉ dành cho dữ liệu draft/empty chưa có lịch sử liên quan. Với user/course/lesson đã có enrollment, submission, certificate, order hoặc audit log, dùng `deleted_at`, `archived`, `hidden` hoặc status nghiệp vụ thay vì hard delete.

### 2.7. `lesson_progress` (Tiến độ bài học)

| Field          | Type      | Constraint                          | Description            |
| :------------- | :-------- | :---------------------------------- | :--------------------- |
| `user_id`      | UUID      | FK → `users.id` ON DELETE RESTRICT  | Học viên               |
| `lesson_id`    | UUID      | FK → `lessons.id` ON DELETE RESTRICT | Bài học                |
...
**Composite Index**: `(lesson_id, completed)` - Optimize [[web/page/admin/admin|admin]] view of lesson completion.
**Business Rule:** Chỉ video/resource lesson được ghi complete trực tiếp vào `lesson_progress`. Quiz completion tính từ `quiz_attempts.passed`; assignment/final_project completion tính từ `[[web/page/instructor/submissions|submissions]].status = approved`.

### 2.8. [[web/page/instructor/submissions|`submissions`]] (Bài nộp)
...
| `user_id`         | UUID      | **Not Null**, FK → `users.id` ON DELETE RESTRICT  | Học viên                          |
| `course_id`       | UUID      | **Not Null**, FK → `courses.id` ON DELETE RESTRICT | Khóa học                          |
| `lesson_id`       | UUID      | **Not Null**, FK → `lessons.id` ON DELETE RESTRICT | Bài tập/Project nào               |
...
| `feedback`        | Text      | -                                                 | Nhận xét của [[web/page/admin/admin|admin]]                |
...
**P1 Rule:** `offline_cash_reward_manual` chỉ là phần thưởng xử lý thủ công/offline bởi admin. Không build self-service cash withdrawal hoặc payment gateway payout trong MVP/P1.
...
| `source_entity_type` | String | -            | Entity nguồn cụ thể: `resource`, `course`, [[web/page|`page`]] |
...
| `landing_page_url` | String | -             | URL landing [[web/page|page]] nguồn              |
...
### 2.12. [[web/page/website/projects|`projects`]] (Triển lãm Dự án - Independent)
...
| :----------- | :-------- | :---------------------------------------------- | :--------------------------------------------- |
| `id`         | UUID      | PK                                              | Định danh duy nhất                             |
| `student_id` | UUID      | **Not Null**, FK → `users.id` ON DELETE RESTRICT | Học viên được ghi chú                          |
...
### 2.14. `resources` (Public resource / [[web/page/website/blog|blog]] hub)
...
| `correct_answer`  | JSONB     | -                           | Đáp án đúng, chỉ [[web/page/admin/admin|admin]]/instructor xem        |
...
- `users.account_balance` tăng đúng bằng `orders.final_amount`; không dùng balance để [[web/page/student/checkout|checkout]] trong MVP/P1.
- Enrollment sinh từ order refunded chuyển `cancelled` để khóa quyền học. Nếu [[web/page/admin/admin|admin]] muốn giữ quyền học, phải tạo override riêng và ghi audit log `enrollment.override_after_refund`.
- User muốn rút số dư phải liên hệ [[web/page/admin/admin|admin]] qua `/contact?type=support`; [[web/page/admin/admin|admin]] xử lý offline và tạo ledger reset balance về `0`.
...
- **`cancelled`**: Hủy bởi [[web/page/admin/admin|admin]]/hệ thống.
...
- `assignment` / `final_project`: có ít nhất một `[[web/page/instructor/submissions|submissions]].status = approved`.
...
- **Free Course** (`level = 'free'`): Truy cập tự do cho mọi user (không yêu cầu [[web/page/student/login|login]]/enrollment). Tuy nhiên, để nhận chứng chỉ, vẫn yêu cầu một bản ghi `enrollment` (có thể tự động tạo).
...
- Trường `users.last_activity_at` được cập nhật mỗi khi user:
  ...
  - Nộp bài ([[web/page/instructor/submissions|`submissions`]])
...
2. Hệ thống ghi:
   ...
   - `revoked_by` $\rightarrow$ `[[web/page/admin/admin|admin]].id`
   - `revoked_reason` $\rightarrow$ lý do do [[web/page/admin/admin|admin]] nhập (ví dụ: "Gian lận bài final project", "Vi phạm chính sách").
3. **Không được** xóa bản ghi [[web/page/website/certificate|certificate]] (giữ lại lịch sử).
...
- Xóa mềm `users` $\rightarrow$ `enrollments`, [[web/page/instructor/submissions|`submissions`]], `orders` **có thể** SET NULL (tùy FK action). Tuy nhiên, với `orders.user_id` là `ON DELETE RESTRICT`, user không thể bị xóa (hard/soft) nếu còn orders liên kết.
...
- **Chỉ [[web/page/admin/admin|admin]]** tạo và quản lý `announcements` với `scope = global` hoặc `scope = course`.
...
- Khi publish announcement, hệ thống tạo `[[web/page/student/notifications|notifications]].type = announcement` cho người nhận phù hợp.
...
- Review public cần `status = published`; [[web/page/admin/admin|admin]] có thể hide/reject nếu spam hoặc vi phạm.

**Certificate Template:**

- Khi cấp [[web/page/website/certificate|certificate]], phải chọn active `certificate_templates`.
...
- [[web/page/instructor/submissions|`submissions`]] và [[web/page/website/projects|`projects`]] là 2 bảng độc lập.
- Không có logic tự động chuyển bài nộp thành project.
- **Luồng thủ công**: Admin xem bài nộp $\rightarrow$ Bấm "Add to Projects" $\rightarrow$ Mở form pre-fill từ submission $\rightarrow$ Tạo bản ghi trong bảng [[web/page/website/projects|`projects`]].
...
- `enrollments(course_id, status, user_id)`: Covering index for [[web/page/admin/admin|admin]] queries
- `enrollments(user_id, status)`: Composite Index (Student's active courses)
- `[[web/page/instructor/submissions|submissions]](user_id, status)`: Composite Index (Student view + Admin queue)
- `[[web/page/instructor/submissions|submissions]](course_id, status)`: Composite Index (Admin review by course)
- `[[web/page/instructor/submissions|submissions]](user_id, lesson_id, attempt_no)`: Unique Index (Submission retry history)
...
- `[[web/page/instructor/submissions|submissions]](user_id)` → `users(id)`
- `[[web/page/instructor/submissions|submissions]](course_id)` → `courses(id)`
- `[[web/page/instructor/submissions|submissions]](lesson_id)` → `lessons(id)`
- `[[web/page/instructor/submissions|submissions]](reviewed_by)` → `users(id)`
...
- `[[web/page/website/projects|projects]](student_id)` → `users(id)`
- `[[web/page/website/projects|projects]](course_id)` → `courses(id)`
- `[[web/page/website/projects|projects]](created_by)` → `users(id)`
...
**Purpose:** Optimize [[web/page/student/dashboard|dashboard]] and [[web/page/admin/admin|admin]] reports for course performance.
...
**Purpose:** Real-time computed check for [[web/page/website/certificate|certificate]] eligibility.
        ...
        WHERE course_id = p_course_id
          ...
          AND (
              ...
              OR (lesson_type IN ('assignment', 'final_project') AND NOT EXISTS (
                  SELECT 1 FROM [[web/page/instructor/submissions|submissions]]
