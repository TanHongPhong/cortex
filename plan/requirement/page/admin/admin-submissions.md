...
| `Request revision` | Set `submissions.status = revision_requested`, học viên được nộp attempt mới |
| `Reject`           | Set `submissions.status = rejected`, không tính hoàn thành |
...
| Projects independence       | Submissions không tự động thành projects; admin chủ động tạo |
...
→ reviewed_by = admin hiện tại (audit trail)
→ updated_by = admin hiện tại
...
→ submissions.status = revision_requested nếu cần học viên sửa và nộp lại
→ submissions.status = rejected nếu từ chối hẳn
→ reviewed_at = hiện tại
→ reviewed_by = admin hiện tại (audit trail)
→ updated_by = admin hiện tại
