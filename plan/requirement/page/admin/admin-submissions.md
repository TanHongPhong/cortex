...
lesson progress
course completion
[[requirement/page/website/certificate|certificate]] eligibility
```
...
- Approved
- Rejected
- Final [[requirement/page/website/projects|projects]] pending
...
| Approved [[requirement/page/instructor/submissions|submissions]]   | Bài đã duyệt                 |
| Rejected [[requirement/page/instructor/submissions|submissions]]   | Bài bị trả lại               |
| Final [[requirement/page/website/projects|projects]] pending | Final project đang chờ duyệt |
...
Trang này cần có filter vì bài nộp có thể tăng nhanh.

| Bộ lọc        | Mục đích                      |
| ------------- | ----------------------------- |
| Search        | Tìm theo tên học viên/email   |
| Status filter | pending / approved / revision_requested / rejected |
...
```text
Hiển thị pending [[requirement/page/instructor/submissions|submissions]] trước.
```
...
Khi [[requirement/page/admin/admin|admin]] bấm `Review`, mở drawer hoặc modal chi tiết.
...
| File/File URL | `attachment_url` external link hoặc `files` metadata nếu upload nội bộ |
...
| `Request revision` | Set `[[requirement/page/instructor/submissions|submissions]].status = revision_requested`, học viên được nộp attempt mới |
| `Reject`           | Set `[[requirement/page/instructor/submissions|submissions]].status = rejected`, không tính hoàn thành |
| `Open demo`        | Mở link demo                                          |
| `View student`     | Xem học viên                                          |
| `View course`      | Xem khóa học liên quan                                |
| `Add to Projects`  | (Optional) Tạo project mới từ submission để trưng bày |
...
| ---------- | --------------- | --------------------------- |
| `pending`  | Chờ [[requirement/page/admin/admin|admin]] duyệt | Chưa tính hoàn thành        |
...
| Final project approved      | Học viên có thể đủ điều kiện [[requirement/page/website/certificate|certificate]]                     |
| Projects independence       | Submissions không tự động thành [[requirement/page/website/projects|projects]]; [[requirement/page/admin/admin|admin]] chủ động tạo |
...
| Certificate relation | Final project approved thì kiểm tra điều kiện [[requirement/page/website/certificate|certificate]] |
...
| Bảng              | Dữ liệu                            |
| ----------------- | ---------------------------------- |
| [[requirement/page/instructor/submissions|`submissions`]]     | Bài nộp                            |
...
| `certificates`    | Kiểm tra điều kiện cấp [[requirement/page/website/certificate|certificate]] |
| `enrollments`     | Kiểm tra khóa học của học viên     |

---

# 14. Cấu trúc dữ liệu [[requirement/page/instructor/submissions|`submissions`]]
...
| `status`          | pending / approved / revision_requested / rejected |
| `feedback`        | Nhận xét [[requirement/page/admin/admin|admin]]                    |
| `submitted_at`    | Ngày nộp                          |
| `reviewed_at`     | Ngày duyệt                        |
| `reviewed_by`     | Admin duyệt                       |
| `created_by`      | Người tạo (audit trail)           |
| `updated_by`      | Người cập nhật cuối (audit trail) |
| `deleted_at`      | Ngày xóa mềm (nếu có)             |
| `created_at`      | Ngày tạo                          |
...
## Lấy danh sách [[requirement/page/instructor/submissions|submissions]]

```text
Lấy [[requirement/page/instructor/submissions|submissions]]
...
Admin bấm Approve
→ confirm
→ [[requirement/page/instructor/submissions|submissions]].status = approved
→ feedback = feedback [[requirement/page/admin/admin|admin]] nhập
→ reviewed_at = hiện tại
→ reviewed_by = [[requirement/page/admin/admin|admin]] hiện tại (audit trail)
→ updated_by = [[requirement/page/admin/admin|admin]] hiện tại

Nếu lesson_type = assignment hoặc final_project:
→ lesson được tính completed
→ recompute required lessons của course
→ nếu đủ điều kiện, set enrollments.status = completed và completed_at = hiện tại
```
...
→ [[requirement/page/instructor/submissions|submissions]].status = revision_requested nếu cần học viên sửa và nộp lại
→ [[requirement/page/instructor/submissions|submissions]].status = rejected nếu từ chối hẳn
→ reviewed_at = hiện tại
→ reviewed_by = [[requirement/page/admin/admin|admin]] hiện tại (audit trail)
→ updated_by = [[requirement/page/admin/admin|admin]] hiện tại
...
→ nếu đủ điều kiện:
   học viên eligible for [[requirement/page/website/certificate|certificate]]
```
...
| `StatusBadge`            | pending / approved / revision_requested / rejected |
...
| Admin xem được danh sách [[requirement/page/instructor/submissions|submissions]]                            |             |
| Pending [[requirement/page/instructor/submissions|submissions]] hiển thị ưu tiên                            |             |
...
| Final project approved có thể kích hoạt eligibility [[requirement/page/website/certificate|certificate]] |             |
...
Nói ngắn gọn: **`/admin/submissions` là nơi [[requirement/page/admin/admin|admin]] duyệt bài tập và final project. Approve thì bài học được tính hoàn thành; reject thì học viên phải sửa và nộp lại. Final project approved là bước quan trọng để xét [[requirement/page/website/certificate|certificate]].**
...
### Relations
- **Outgoing Links:** [[requirement/page/admin/admin|Admin Dashboard — Requirement]], [[requirement/page/instructor/submissions|/instructor/submissions — Duyệt bài nộp]], [[requirement/page/website/certificate|/certificate — Trang chứng chỉ]], [[requirement/page/website/projects|/projects — Trang dự án học viên]]
